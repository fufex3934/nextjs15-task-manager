...
export default function Home() {
  ...
  const fetchTasks = async () => {
    const res = await fetch("/pages/api/tasks"); // We use this method to call API with desired routes, and store API REsponse on res Variable
      ...
      <div className="filter-section grid grid-flow-col px-3 space-x-2 pb-4">
        <button
          className={`filter-btn px-4 py-2 text-xs text-gray-600 ${
            filter === "all"
              ? "border-2 border-gray-600 rounded-full text-gray-800"
              : "border-2 border-gray-700 rounded-full text-gray-600"
          }`}
